...
struct utmp *
getutent ()			/* returns next utmp file entry */
{
...
char *daynames[] =
  { "SU", "MO", "TU", "WE", "TH", "FR", "SA", "WK", "AL", NULL };
     ...
     char *argv[];
{
  ...
  if (argc != 1 && argc != 3)
    {
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_ERR,
	      "Incorrect invocation of timeoutd (argc=%d) by UID %d.", argc,
	      getuid ());
      closelog ();
      exit (5);
    }
   ...
   */
#ifndef DEBUG			/* Want core in current dir */
  if (chdir ("/dev"))
    {
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
  ...
  if (argc == 3)
    {
      ...
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (SYSLOG_DEBUG,
	      "Running in user check mode.  Checking user %s on %s.", argv[1],
	      argv[2]);
      closelog ();
#endif
...
#ifdef DEBUG
	  openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
	  syslog (SYSLOG_DEBUG, "User %s on %s passed login check.", argv[1],
		  argv[2]);
	  closelog ();
#endif
	  free_wtmp ();
...
#ifndef DEBUG			/* Debugger stays in foreground */
  ...
  while (1)
	...
	reread_config (SIGHUP);
      ...
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (SYSLOG_DEBUG,
	      "Finished checking utmp... sleeping for 1 minute.");
      closelog ();
#endif
      int nap = 60;
      sleep (nap);
...
read_wtmp (time_t since)
{
  ...
  while (fread (&ut, sizeof (struct utmp), 1, fp) == 1)
	...
	{
	  if ((ut_list_p =
	       (struct ut_list *) malloc (sizeof (struct ut_list))) == NULL)
  ...
  while (wtmplist)
    {
      ...
      printf ("%d:%d %s %s %s\n",
	      ...
	      LOGIN_PROCESS ? "login" : wtmplist->elem.ut_type ==
	      BOOT_TIME ? "reboot" : "logoff"
#else
	      ""
#endif
	);
     ...
     char *time_str;
{
  ...
  if ((*t =
       (struct time_ent *) malloc (ar_size * sizeof (struct time_ent))) ==
      NULL)
      ...
      if (*p)
	{
	  if (strlen (p) != 9 || p[4] != '-')
	    {
	      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
	      syslog (LOG_ERR,
		      "Malformed time (%s) in time field of config file (%s).  Entry ignored.",
		      p, CONFIG);
	      closelog ();
	      (*t)->days = 0;
	      return;
	  ...
	  if ((te->starttime == 0 && strncmp (p, "0000-", 5))
	      || (te->endtime == 0 && strcmp (p + 5, "0000")))
	    {
	      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
	      syslog (LOG_ERR,
		      "Invalid range (%s) in time field of config file (%s).  Entry ignored.",
		      p, CONFIG);
	      closelog ();
	      (*t)->days = 0;
	      return;
...
read_config ()
{
  ...
  while (fgets (line, 256, config_file) != NULL)
      ...
      if (*lstart)
	{
	  ...
	  if ((config[i] =
	       (struct config_ent *) malloc (sizeof (struct config_ent))) ==
	      NULL)
	    bailout ("Out of memory", 1);
	  config[i]->times = NULL;
	  config[i]->ttys = NULL;
...
char *build_devname (const char *tty)
{
  TRACE(build_devname);
  DUMPS(tty);
  ...
  if (strstr(dev, "/dev/") == NULL)
    sprintf(ret, "/dev/%s", dev);
  else
    sprintf(ret, "%s", dev);
  return ret;
}

void ttywarn (char *tty, int time_remaining, char *user, char *host)
{
  int fd = -1;
  FILE *ttyf = NULL;
  char *devname = NULL;
  ...
  devname = build_devname(tty);
  if ((fd = open (devname, O_WRONLY | O_NOCTTY | O_NONBLOCK)) < 0 ||
  ...
  else 
    {
      fprintf (ttyf,
	       ...
	       time_remaining, time_remaining == 1 ? "" : "s",
	       limit_names[limit_type]);
    }
  if (ttyf != NULL) fclose (ttyf);
  if (fd != -1) close (fd);
}
...
#ifdef DEBUG
  openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
  syslog (SYSLOG_DEBUG, "Checking user %s group %s tty %s.", user,
	  gr->gr_name, tty);
  closelog ();
#endif
  ...
  while (config[configline])
	  ...
	  while (*p && !groupmatch)
	    {
	      ...
	      if (!strcmp (*p, user))
		groupmatch =
		  chkmatch (secgr->gr_name, config[configline]->groups);
	      p++;
	    }
	  /*
	     free(gr);
      ...
      if (timematch && ttymatch && usermatch && groupmatch)
	{
	  ...
	  syslog (SYSLOG_DEBUG,
		  ...
		  config[configline]->sessmax, daytime,
	  ...
	  if (config[configline]->sessmax > 0
	      && session >= config[configline]->sessmax)
	    return SESSMAX;

	  limit_type = DAYMSG;
	  if (config[configline]->daymax > 0
	      && daytime >= config[configline]->daymax)
	    return DAYMAX;
...
check_idle ()			/* Check for exceeded time limits & logoff exceeders */
{
  ...
  if (aktconfigline > 0)
    {				/* > 0 if user is not in config */
#ifdef DEBUG
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (SYSLOG_DEBUG,
	      "User %s or * not in config -> No restrictions. Not checking %s on %s",
	      user, user, dev);
      closelog ();
#endif
  ...
  switch (chk_timeout (user, dev, host, idle, sesstime))
    ...
    case NOLOGIN:
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
#ifdef DEBUG
      syslog (LOG_NOTICE, "NOLOGIN period reached for user %s@%s. (pid %d)",
	      user, host, utmpp->ut_pid);
#else
      syslog (LOG_NOTICE, "NOLOGIN period reached for user %s %s", user,
	      host);
#endif
      closelog ();
      killit (utmpp->ut_pid, user, dev, host);
     ...
     char *host;
{
  ...
  /* Tell user which limit they have exceeded and that they will be
     logged off. Trim the tty name first. */
  devname = build_devname(dev);
  if ((tty = open (devname, O_WRONLY | O_NOCTTY | O_NONBLOCK)) < 0)
    {
      int err = errno;
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
  ...
  /* check if the pid is sshd. If so, get PID of the child process
     (another ssh, owned by the user). Test reverse if this child
     process is also ssh and owned by the user we want to log
     out. (because we don't want to slay another user ;) */
  cpid = getcpid (pid);
#ifdef DEBUG
  openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
  syslog (LOG_NOTICE, "I am at killit() pid=%d user=%s child=%d line %d", pid,
	  user, cpid, __LINE__);
  closelog ();
#endif
  ...
  if (chk_ssh (pid) && chk_ssh (cpid) && !strcmp (getusr (cpid), user))
    {
      ...
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_NOTICE,
	      "User %s (pid:%d, cpid:%d) logged in via ssh from %s.", user,
	      pid, cpid, host);
      closelog ();
#endif
      pid = cpid;
  ...
  if ((pw = getpwnam (user)) == NULL)
    {
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_ERR,
	      "Could not log user %s off line %s - unable to determine uid.",
	      user, dev);
      closelog ();
    }
  if (setuid (pw->pw_uid))
    {
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_ERR,
	      "Could not log user %s off line %s - unable to setuid(%d).",
	      user, dev, pw->pw_uid);
      closelog ();
    }
  kill (-1, SIGHUP);
  ...
  if (ioctl (fd, TIOCGETD, &disc) < 0)
    {
      ...
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_WARNING,
	      "Could not get line discipline for %s - idle limits will be enforced.",
	      d);
      closelog ();
      return N_TTY;
    }
...
#ifdef DEBUG
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_DEBUG, "LOCAL Xsession detected. device=%s host=%s", dev,
	      host);
      closelog ();
#endif
...
#ifdef DEBUG
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_DEBUG, "REMOTE Xsession detected. device=%s host=%s", dev,
	      host);
      closelog ();
#endif
...
#ifdef DEBUG
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_DEBUG, "NO xsession detected. device=%s host=%s", dev,
	      host);
      closelog ();
#endif
...
*/
int
chk_xterm (dev, host)		/* returns 1 when dev and host seem to be a xTERM. */
     char *dev, *host;
{
...
#ifdef DEBUG
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_DEBUG, "XTERM detected. device=%s host=%s Ignoring.", dev,
	      host);
      closelog ();
#endif
      return 1;
...
#else
  openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
  syslog (LOG_ERR, "Would normally logoff user %s running X (kill PID %d)",
	  user, pid);
  closelog ();
#endif
}
...
int
chk_ssh (pid)			/* seppy; returns true if pid is sshd, otherwise it returns false */
     pid_t pid;
{
  TRACE (chk_ssh);
  DUMP (pid);
  ...
  if (!proc_file)
    {
      openlog ("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
      syslog (LOG_WARNING,
	      "chk_ssh(): PID %d does not exist. Something went wrong. Ignoring.",
	      pid);
      closelog ();
      return 0;
    }
...
char *
getusr (pid)			/*seppy; returns the name of the user owning process with the Process ID pid */
     pid_t pid;
{
  TRACE (getusr);
  DUMP (pid);
     ...
     pid_t ppid;
{
  ...
  while ((cont = readdir (proc)) != NULL)
    if (cont->d_type == 4 && isdigit (cont->d_name[0]))
      {				/* check only PIDs */
